...
if platform != 'darwin':
    print('WRAN: using Agg backend', platform)
    mpl.use('Agg')  # No display
# elif int(os.environ.get('FORCE_AGG', 0)):
...
def default_creator(plt_):
    fig = plt_.figure(dpi=100)
    return fig, plt_
...
class PlotToArray(object):
    ...
    def prepare(self, creator=None):
        if not creator:
            creator = default_creator
        self.fig, returnable = creator(plt)
        # self.fig = plt.figure(dpi=100)
        return returnable  # plt by default

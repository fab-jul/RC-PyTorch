...
from vis import figure_plotter
...
class _GlobalStepDependable(object):
    ...
    def enable(self, prefix, global_step):
        self._enable(prefix, global_step)
        yield
        self.disable()
...
class Summarizer(_GlobalStepDependable):
    ...
    def register_figures(self, prefix, creators, fig_creator=figure_plotter.default_creator):
        """
        :param prefix: Prefix to use in TensorBoard
        ...
        if prefix == self.enabled_prefix:
            for name, creator in creators.items():
                with self.sw.add_figure_ctx(prefix + '/' + name, self.global_step, fig_creator) as plt:
                    creator(plt)

    def register_images(self, prefix, imgs, normalize=False, only_once=False):  # , early_only=False):
        """
        :param prefix: Prefix to use in TensorBoard
        ...
        if prefix == self.enabled_prefix:
            for name, img in imgs.items():
                tag = prefix + '/' + name
                if only_once:
                    if tag in self.only_once:
                        continue
                    self.only_once.add(tag)
                img = _convert_if_callable(img)
                if normalize:
                    img = normalize_to_0_1(img)
                self.sw.add_image(tag, img, self.global_step)


def _convert_if_callable(v):
    if hasattr(v, '__call__'):  # python 3 only
        # Don't need gradients when calculating summaries!
        with torch.no_grad():
            return v()
    return v

...
from compressor import open_safe
from dataloaders.cached_listdir_imgs import Images
...
class IndexImagesDataset(Dataset):
    ...
    def write_file_names_to_txt(self, log_dir):
        ...
        with open(os.path.join(log_dir, 'train_imgs.txt'), 'w') as fout:
            fout.write(f'{len(self.files)} images, {self.id}\n---\n')
            fout.write('\n'.join(map(os.path.basename, self.files)))

    def __init__(self, images, to_tensor_transform):
        """
        :param images: Instance of Testset or Images (namedtuple)
        ...
        if isinstance(images, Testset):
            self.files = images.ps
            self.id = images.id
        elif isinstance(images, Images):
            self.files = images.ps
            self.id = images.id
        else:
            raise ValueError('Expected Testset or Images, got images={}'.format(images))
    ...
    def __getitem__(self, idx):
        path = self.files[idx]
        return self.dict_from_path(idx, path)

    def dict_from_path(self, idx, path):
        pil = open_safe(path)
        pil = pil.convert('RGB')
        raw = self.to_tensor_transform(pil)  # 3HW uint8s
        return {'raw': raw}

    def load_transform_img(self, img_p, center_crop=False):
        with open(img_p, 'rb') as f:
            pil = Image.open(f).convert('RGB')
            if center_crop:
                pil = F.center_crop(pil, center_crop)
            return self.to_tensor_transform(pil)  # 3HW uint8s


class IndexDataset(Dataset):
    def __init__(self, ds):
        raise DeprecationWarning  # this was used for checkerboards when we needed indices
        self.ds = ds
    ...
    def __getitem__(self, idx):
        return {'idx': idx, 'raw': self.ds[idx]}

    def __str__(self):
        return f'IndexDataset({str(self.ds)})'
...
class ImagesCached(object):
    ...
    def get_images_sorted_cached(self):
        key = self.images_spec, (self.min_size if '32' not in self.images_spec else None)
    ...
    def _iter_imgs_unordered(images_spec):
        ...
        for root, _, fnames in os.walk(images_spec, followlinks=True):
            for fname in fnames:
                if total_files_traversed > 0 and total_files_traversed % 100 == 0:
                    print(f'\rTraversed {total_files_traversed} files', end='')
                if has_image_ext(fname):
                    ...
                    yield p
        print(f'\rTraversed {total_files_traversed} files')

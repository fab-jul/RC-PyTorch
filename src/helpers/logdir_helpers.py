...
def create_unique_log_dir(config_rel_paths, log_dir_root, line_breaking_chars_pat=r'[-]',
    ...
    postfix_dir_name = get_log_dir_name(config_rel_paths, line_breaking_chars_pat, postfix, restore_dir, strip_ext)

    return _mkdir_threadsafe_unique(log_dir_root, datetime.now(), postfix_dir_name)


def get_log_dir_name(config_rel_paths, line_breaking_chars_pat=r'[-]',
                     postfix=None, restore_dir=None, strip_ext=None):
    if strip_ext:
...
def parse_log_dir(log_dir, configs_dir, base_dirs, append_ext=''):
    ...
    def get_real_path(base, prepped_p):
        ...
        if len(glob_matches_of_same_len) != 1:
            raise ValueError('Cannot find config on disk: {} (matches: {}; {})'.format(
                    p_glob, glob_matches_of_same_len, os.getcwd()))
...
def split_log_dir(log_dir):
    # extract tuple ( {log_date}, {configs ...} ) from LOG_DIR/{log_date} {configs ...}
    log_dir = os.path.basename(log_dir)
    possible_log_date, possible_log_name = log_dir.split(' ', 1)
    if not is_log_date(possible_log_date):
        raise ValueError('Invalid log dir: {}'.format(log_dir))
    return possible_log_date, possible_log_name


def log_date_from_log_dir(log_dir):
    # extract {log_date} from LOG_DIR/{log_date} {configs ...}
    return split_log_dir(log_dir)[0]


def log_name_from_log_dir(log_dir):
    # extract {configs ...} from LOG_DIR/{log_date} {configs ...}
    return split_log_dir(log_dir)[1]

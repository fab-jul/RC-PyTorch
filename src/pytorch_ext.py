...
class FeatureMapSaver(nn.Module):
    def __init__(self, log_dir, feature_name):
        super(FeatureMapSaver, self).__init__()
    ...
    def _save(self, x):
        ...
        grid = [x[c, ...].unsqueeze(0) for c in range(C)]
        # grid = vis.grid.prep_for_grid(grid)
        grid = torchvision.utils.make_grid(grid, nrow=grid_size)
        grid = normalize_to_0_1(grid).mul_(255.).to(torch.uint8)
        grid = tensor_to_np(grid.permute(1, 2, 0))
        p = os.path.join(self.log_dir, '{}_{:010d}.png'.format(self.feature_name, self.step))
        print('saving', p)
        Image.fromarray(grid).save(p)
...
def one_hot(x, L, Ldim, dtype=torch.float32):
    """ add dim L at Ldim """
    ...
    oh = torch.zeros(*out_shape, dtype=dtype, device=x.device)
...
class FeatureMapSaverSequential(nn.Sequential):
    def __init__(self, *args, saver=None, idx=None):
        """
        :param args:
        :param saver:
        :param idx:  -1 means final, -2 means before final, etc.
        """
        super().__init__(*args)
    ...
    def forward(self, x):
        if not self._saver:
            return super(FeatureMapSaverSequential, self).forward(x)

...
def _get_C_cur(targets, means_c, log_scales_c):  # NKHWL
    ...
    # NKHWL', distance of every predicted mean value to every target
    centered_targets = (targets - means_c.unsqueeze(-1))
    # NKHWL', cdf evaluated at centered_targets
...
def _renorm_cast_cdf_(cdf, precision):
    """
    Move CDF from [0,1] to int16. Note that in the C++ code, this will then be interpreted as uint16.
    We also add range(Lp) to ensure each of the Lp symbols gets an unique value in [0, ..., 2^16].
    Otherwise, we could not decode deterministically.
    """
    # TODO: range may go slightly above 1
    Lp = cdf.shape[-1]
    finals = 1  # NHW1
    # RENORMALIZATION_FACTOR in cuda: 2^16
    f = torch.tensor(2, dtype=torch.float32, device=cdf.device).pow_(precision)
    cdf = cdf.mul((f - (Lp - 1)) / finals)
    cdf = cdf.round()
